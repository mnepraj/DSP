...
\author{Prajwal M EE23BTECH11217$^{*}$ %<-this  stops a space
...
\end{lstlisting}
	...
	\item The system with $h(n)$ is defined to be stable if
	      ...
	      \solution
	      \begin{align}
		      \sum_{n=-\infty}^{\infty} h(n) & = \sum_{n=-\infty}^{\infty} \left(\left(-\frac{1}{2}\right)^n u(n) + \left(-\frac{1}{2}\right)^{n-2} u(n-2)\right) \\
		                                     & = \sum_{n=0}^{\infty} \left(-\frac{1}{2}\right)^n + \sum_{n=2}^{\infty} \left(-\frac{1}{2}\right)^{n-2}            \\
...
\end{lstlisting}
	...
	\item Compute
	      ...
	      %
	      \begin{lstlisting}
wget https://raw.githubusercontent.com/mnepraj/DSP/master/filter/codes/ynconv.py
\end{lstlisting}
	      \begin{figure}[!ht]
		      \centering
...
\begin{enumerate}[label=\thesection.\arabic*]
	...
	\item Compute
	      ...
	      %
	      \begin{lstlisting}
wget https://raw.githubusercontent.com/mnepraj/DSP/master/filter/codes/DFT.py
\end{lstlisting}
	      This code verifies the result by plotting the obtained result with the result obtained by IDFT.
		  \begin{figure}[h]
			\centering
			\includegraphics[width=\columnwidth]{figs/ynIDFT.png}
			\caption{$y(n)$ obtained from IDFT}
			\label{fig:yn_verf_6.3}
		\end{figure}
	\item Repeat the previous exercise by computing $X(k), H(k)$ and $y(n)$ through FFT and IFFT.
	      ...
	      %
	      \begin{lstlisting}
wget https://raw.githubusercontent.com/mnepraj/DSP/master/filter/codes/FFT.py
\end{lstlisting}
	      This code verifies the result by plotting the obtained result with the result obtained by IFFT.
	      \begin{figure}[h]
		      \centering
		      \includegraphics[width=\columnwidth]{figs/ynIFFTverified.png}
...
\lstinputlisting{./codes/filtfiltreplaced.py}
